...
func (l *tunLink) Capabilities() stack.LinkEndpointCapabilities {
	// No checksum offload, no capability flags: netstack computes checksums.
	return 0
}
...
func (l *tunLink) inject(proto tcpip.NetworkProtocolNumber, payload []byte) {
	...
	pkt := stack.NewPacketBuffer(stack.PacketBufferOptions{
		Payload: buffer.View(payload).ToVectorisedView(),
	})
	l.dispatch.DeliverNetworkPacket(proto, pkt)
}

// WritePackets drains the netstack's outbound queue into the TUN device.
func (l *tunLink) WritePackets(pkts stack.PacketBufferPtr) (int, tcpip.Error) {
	n := 0
	for pkt := pkts; pkt != nil; pkt = pkt.Next() {
		data := pkt.ToView().AsSlice()
		if _, err := l.tun.Write(data); err != nil {
